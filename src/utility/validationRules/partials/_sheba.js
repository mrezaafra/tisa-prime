...
let pattern_code = /IR[0-9]{2}([0-9]{3})[0-9]{19}/;
...
export const bankRecognizer = function (str) {
  if (!validate(str)) {
    return false;
  }
  let res = pattern_code.exec(str);
  let code = res[1];
  if (!banksHash[code]) {
    return false;
  }
  let bank = banksHash[code];
  let result = {
...
export const isShebaValid = validate;
